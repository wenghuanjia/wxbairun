        ...
        if (result.confirm) {
          wx.canvasToTempFilePath({
            ...
            success(res) {
              ...
              //上传
              wx.uploadFile({
                url: 'https://bairunminiapp.gzwmdy.cn/user/upload/one', // 仅为示例，非真实的接口地址
                filePath: res.tempFilePath,
                name: 'file',
                header: {
                ...
                success(img) {
                  ...
                  if (img.statusCode === 200 && data.status === 1) {
                    wxUtils.showToast('上传成功');
                    _this.setData({
                      uploadImgUrl: data.data.url
                    })
                  } else {
                    wxUtils.showToast('图片上传有误，请稍后重试!')
                  }
                },
                fail: err => {
                  console.log(err);
